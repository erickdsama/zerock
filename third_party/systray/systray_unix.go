...
func argbForImage(img image.Image) []byte {
	...
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
			// zerock patch: RGBA() returns 16-bit premultiplied values, and
			// keeping their low byte scrambles every partly transparent
			// pixel. StatusNotifierItem wants straight 8-bit ARGB.
